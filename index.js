...
module.exports = function(params, done) {
    ...
    kss.traverse(options.src, { mask: '*.css' }, function(err, styleguide) {
        if (err) throw err;

        var sections = styleguide.section('*.'),
        sectionCount = sections.length,sectionRoots = [],
        rootCount,
        currentRoot,
        childSections = [],
        pages = {},
...
var updateCollection = function(col, page, context) {
  if(!context[col.name]) {
    return col;
  }
...
var getParentRef = function(str) {
    var a = str.indexOf('.');
    var b = str.lastIndexOf('.');
    if(a < 0) return null;
    else if(a == b) return str.substring(0,1);
    else return str.substring(a-1, b);
}

// Convert an array of `KssSection` instances to a JSON object.
var jsonSections = function(sections) {
...
var jsonModifiers = function(modifiers) {
